...
class SaleOrder(models.Model):
    ...
    def action_create_rfq(self):
        ...
        if self.user_id:
            ...
            login_employee = employee_id = self.env['hr.employee'].sudo().search([('user_id','=',self.env.uid)])
            if login_employee and login_employee.parent_id and login_employee.parent_id.user_id:
                activity_user_id = login_employee.parent_id.user_id.id
            elif employee_id and employee_id.parent_id and employee_id.parent_id.user_id:
                activity_user_id = employee_id.parent_id.user_id.id
            else:
                for loop in user:
